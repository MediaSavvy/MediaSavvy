...
:::warning Note:
It kills running malicious processes, removes policies in the registry that prevent normal OS operation and repairs file extension hijacks. **Note that it may not terminate newer malware as its last update was in 2018, however, other fixes regarding the registry and file extensions still work depending on the malware.**
:::
...
Then run [MRT](https://www.microsoft.com/en-us/download/details.aspx?id=16) and reboot by holding down shift, start Windows in safe mode. If the problem reappears, you'll need to clean install or check if it is a hardware problem.
...
Install a better anti-virus. See recommendations above.
